...
function handleComputerArrowCollision() {
  //Write code to detect collision between computer arrow and opponent
    ...
    for (var i=0;i < computerArrows.length;i++){
      var basecCollision = Matter.SAT.collides(
        computerArrows[i].body,
        playerBase.body 
      );
    
      var archercCollision = Matter.SAT.collides(
        computerArrows[i].body,
        playerArcher.body 
      );
    
      var computercCollision = Matter.SAT.collides(
        computerArrows[i].body,
        player.body 
      );
    
      if(
        basecCollision.collided||
        archercCollision.collided||
        computercCollision.collided
        ) {
